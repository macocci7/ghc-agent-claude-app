...
{
    $parts = explode('.', $key);
    $config = require __DIR__ . '/html/config/' . $parts[0] . '.php';
...
function runMigrations(): void
{
    try {
        ...
        // Get migration files
        $migrationFiles = glob(__DIR__ . '/html/database/migration/*.php');
        sort($migrationFiles);
        ...
        foreach ($migrationFiles as $file) {
            ...
            $className = ucfirst(str_replace('.php', '', preg_replace('/^\d{4}_\d{2}_\d{2}_\d{6}_/', '', $filename)));
